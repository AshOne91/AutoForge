...
- [x] first KIS trading Blueprint slice: automated read-only domestic daily
  OHLCV collection uses the independent `market_history` module, shared KIS
  token/Redis contracts, generated global `automation` persistence, an
  operator-protected proof path, Durable Job/worker messaging, and a generated
  Airflow schedule with deployment-owned JSON payload injection. The preserved
  `market_data` baseline is not rewritten. This public market data is not
  user-sharded and the slice adds no order, execution, portfolio mutation, or
  risk behavior.
- [~] KIS trading Blueprint validation: 시장 데이터 수집, KIS 인증·공유 token
  ...
  연결을 통한 보유종목 조회, portfolio snapshot, order/execution, risk limit와
  감사 이력은 남아 있다.
