...
- opt-in RAG infrastructure with Qdrant, Ollama, and one selectable search backend
  ...
  stable `/api/tags` readiness response continued, then restarted that member,
  confirmed all three services were healthy, and rechecked the same proxy endpoint.
  No model is downloaded or shared automatically; actual inference failover requires
  an operator to prepare the selected model in every member volume. Both local
  standalone search paths are runtime-verified.
