...
application의 `/health` Compose healthcheck은 생성된 내부 PostgreSQL 대상의
TCP 연결 가능 여부와 Redis 연결을 확인한다. Redis Cluster 모드에서는
`require_full_coverage=True`와 `PING`으로 전체 슬롯 커버리지를 확인한다.
외부 관리형 저장소의 인증된 SQL 쿼리까지 이 로컬 healthcheck 계약에 포함하지
않는다.
