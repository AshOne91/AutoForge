...
이 Level은 앞선 서버를 실행하기 위한 단계가 아니다. 로그인·Quest Board 실습을 끝낸
주니어가 AutoForge 자체를 확장해야 할 때 읽는 **참고 단계**다. 지금 Plugin을 만들거나
Git 자동화를 켜라는 뜻이 아니므로, 반복되는 생성·검증 규칙이 실제로 생기기 전에는
다음 "공통 클리어 테스트"로 넘어간다.
