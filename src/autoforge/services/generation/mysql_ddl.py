...
class MySQLDDLGenerator:
    ...
    def render(self, specification: ModuleSpec) -> dict[PurePosixPath, str]:
        ...
        for migration in database.migrations:
            ...
            target_table = (
                migration.create_tables[0]
                if migration.create_tables
                else migration.add_columns[0].table
            )
            path = PurePosixPath(
                "database",
                placements[target_table].mode.value,
                f"{migration.revision:04d}_{specification.module.name}_{migration.name}.sql",
            )
            rendered[path] = self._render_statements(
                specification, placements[target_table].mode, statements
            )
        return rendered
